...
## File descriptor and PID limits
I left these resource limits at the end since I am the least familiar with them. From what I understand by reading online and chatting with Gemini, nginx only has a main and a worker process, so we can set a low PID limit. For the file descriptor limit, nginx opens 2 sockets (one between itself and the client and one between itself and the upstream, the Go container) for every concurrent connection. I should increase these based on the number of desired concurrent connections for our application.
For the Go API, the file descriptor limit should be similar to nginx since there are 2 sockets (one between itself and PokeAPI, one between itself and nginx). However, Go spawns a new process for each thread it uses. So the process limit should be scaled up based on the number of CPU cores provided to the application, if not limited by the `GOMAXPROCS` environment variable.
A 256 limit for both should protect the system against fork-bombs while leaving breathing room for the applications running inside the containers.
For the file descriptor limits, the physical limit for concurrent connections should be 65536, so we can increase the `nofile` limit to that number to allow more than 30000 concurrent connections.
